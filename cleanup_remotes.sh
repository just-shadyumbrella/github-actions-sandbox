...
for remote in $remotes; do
    echo
    echo -e "Purging benchmark file from remote \"$remote\""
    rclone purge -P "${remote}benchmark_test/"
    
    # Check the exit status of the command
    if [ $? -eq 0 ]; then
        echo "${GREEN}Successfully purging \"$remote\"${NC}"
    else
        echo "${RED}Error purging \"$remote\"${NC}" >&2
    fi
done

echo
echo -e "${YELLOW}All remotes cleaned up!${NC}"
